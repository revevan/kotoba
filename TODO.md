...
- [ ] Shorts → YouTube: **audit follow-up answered 2026-08-26** (sent upload script + channel link + full upload log per their 2026-08-25 request) — now waiting on their verdict; nothing is blocked meanwhile: publishing is live (first 2 shorts went public on schedule Aug 24/25), 14 uploaded/scheduled through Sep 10, next batch approved. If they ask for a screencast: 2-min QuickTime of `pnpm run upload-shorts`. Pipeline fully automated; rejected shorts via `pnpm run reviews`. Monthly check-in: Studio retention data ~end of Sep → double down or adjust formats
- [ ] **Outreach gate — wife re-check of the 46 fixed n5-starter flags.** All 46 fixed, deployed (4710247b, CI green, live data verified) and re-queued 2026-08-22: ~35 sentences replaced per her notes, glosses (mom/dad/(someone's) older brother/sister), 7 word-intro + 6 per-sentence `speak` pitch overrides (new per-sentence `speak` field in gen-audio/publish-sentences is the iteration knob if any still sound off). 224 clips regenerated → R2, cache purged. Her queue: 46 re-checks + fresh review of words 106–121 (last unreviewed starter slice). When verified → outreach unblocks (JoyOfJapanese, Speechling, Guide2Fluency pitches + dev.to builder post; AllLanguageResources follow-up ~2026-08-31)
