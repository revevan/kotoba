...
// A prompt that couldn't load: the session is parked, not listening, and will
// replay it — the phase badge ("QUESTION") would suggest an answer is expected.
const OFFLINE_BADGE = { text: 'CONNECTION LOST — retrying…', cls: 'offline' };
...
export function SessionScreen() {
  ...
  const badge = s.offline ? OFFLINE_BADGE : ((s.srErrorReveal ? SR_ERROR_BADGE[s.phase] : undefined) ?? PHASE_BADGE[s.phase]);
  ...
  return (
    <div class={`screen session ${badge.cls}`}>
      ...
      {s.offline && <p class="hint">Audio couldn’t load. The question replays when the connection is back. Nothing is graded meanwhile.</p>}
