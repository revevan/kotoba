...
/** "Connection lost. I'll keep trying." — the one clip that must play with NO
 *  network, so it ships with the app shell (public/cues/, precached), never
 *  under AUDIO_BASE/R2. Generated by tools/gen-audio.ts (CUES). */
export const connectionLostCue = (): ClipItem[] => [{ src: `${import.meta.env.BASE_URL}cues/connection-lost.mp3` }];
